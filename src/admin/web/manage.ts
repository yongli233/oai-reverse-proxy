...
router.post("/rotate-token/:token", (req, res) => {
  ...
  req.session.flash = {
    type: "success",
    message: "User's token was rotated.",
  };
...
router.get("/key-manager", (req, res) => {
  ...
  const keys = keyPool.list().sort((_a, _b) => {
...
router.post("/key-manager/add-keys", (req, res) => {
  const keys = req.body.keyInput.trim().replace(/[\n\r]/g, "");
  const keyArray = keys.split(",");
  const service = req.body.service;

  const addedKeys = keyPool.addKeys(service, keyArray);
  keyPool.recheck(service);

  const urlSearch = new URLSearchParams();
  urlSearch.append("service", service);
  urlSearch.append("addedKeys", addedKeys.toString());

  return res.redirect(`/admin/manage/key-manager?${urlSearch}`);
});

router.post("/key-manager/delete-keys", (req, res) => {
  const mode = req.body.hash;
  const service = req.body.service;

  const removedKeys = keyPool.removeKeys(service, mode);

  const urlSearch = new URLSearchParams();
  urlSearch.append("service", service);
  urlSearch.append("removedKeys", removedKeys.toString());
