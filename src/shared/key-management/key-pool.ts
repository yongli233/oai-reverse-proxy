...
export class KeyPool {
  ...
  public addKeys(service: LLMService, keys: string[]) {
    ...
    for (const key of keys) {
      if (keyProvider.addKey(key, false)) addedKeys++;
    }

    return addedKeys;
  }

  public removeKeys(service: LLMService, mode: string) {
    const keyProvider = this.getKeyProvider(service);
    let removedKeys = 0;

    switch (mode) {
      case "revoked": {
        const revoked = keyProvider.list().filter((key) => key.isRevoked);
      ...
      case "overQuota": {
        const overQuota = keyProvider.list().filter((key) => {
          if ("isOverQuota" in key && typeof key.isOverQuota === "boolean")
            return key.isOverQuota;
          return false;
        });

        for (const key of overQuota) {
          if (keyProvider.removeKey(key.hash)) removedKeys++;
        }
        break;
      }
      ...
      default: {
        if (keyProvider.removeKey(mode)) removedKeys++;
